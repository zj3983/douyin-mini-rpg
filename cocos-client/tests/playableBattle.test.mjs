...
async function loadPortraitBootstrapHarness() {
  const stubUrl = moduleUrl(`
    export class Node {
      constructor(name = '') { this.name = name; this.active = true; this.listeners = new Map() }
      ...
      off(eventName, callback, target) {
        const listeners = this.listeners.get(eventName) ?? []
        this.listeners.set(eventName, listeners.filter((entry) => entry.callback !== callback || entry.target !== target))
      }
      emit(eventName, payload) {
        for (const entry of [...(this.listeners.get(eventName) ?? [])]) entry.callback.call(entry.target, payload)
      }
      listenerCount(eventName) { return (this.listeners.get(eventName) ?? []).length }
    }
    export class Component { constructor() { this.node = new Node('BootstrapHost') } schedule() {} unschedule() {} }
    ...
    export class Camera { static ProjectionType = { ORTHO: 0 } }
    export class Canvas {}
    export class Color { constructor() {} }
    export class Graphics {}
    export class JsonAsset {}
    export class ProgressBar {}
    export class Sprite { static SizeMode = { CUSTOM: 0 } }
    ...
    export const ResolutionPolicy = { SHOW_ALL: 0, FIXED_WIDTH: 1 }
    export const resources = { load() {} }
    export const view = { getFrameSize: () => ({ width: 390, height: 844 }), on() {}, off() {}, setDesignResolutionSize() {} }
    ...
    export const ORDINARY_ENEMY_FRAME_HEIGHT = 1
    export const ORDINARY_ENEMY_FRAME_WIDTH = 1
    export const PLAYER_DISPLAY_SCALE = 1
    export const PLAYER_FRAME_HEIGHT = 1
    export const PLAYER_FRAME_WIDTH = 1
    export const BOSS_HAZARD_POOL_CAPACITY = 1
    export const computeBattleViewportState = () => ({ resolution: { mode: 'fixed-width', designWidth: 750, designHeight: 1334 }, layout: {} })
    export const computeDungeonEntryNavLayout = () => ({ navigation: { centerY: 0 }, label: { height: 1, centerY: 0 }, status: { height: 1, centerY: 0 } })
    export const computeWorldStageSelectLayout = () => ({})
    export const createDefaultViewportMetricsProvider = () => ({ read: () => ({}), subscribe: () => () => {}, destroy() {} })
  ...
  javascript = javascript.replace(/from ['"][^'"]+['"]/g, `from '${stubUrl}'`)
  return { ...await import(moduleUrl(javascript)), ...await import(stubUrl) }
}

test('world stage overlay event harness advances only unlocked stages and owns its listener lifecycle', async () => {
  const { Button, Node, PortraitBattleBootstrap } = await loadPortraitBootstrapHarness()
  const bootstrap = new PortraitBattleBootstrap()
  const selector = new Node('WorldStageSelectRoot')
  const battle = new Node('BattleRoot')
  const close = new Node('WorldStageCloseButton')
  const entry = new Node('WorldStageEntryButton')
  const items = Array.from({ length: 10 }, (_, index) => new Node(`WorldStageItem${index + 1}`))
  const buttons = Array.from({ length: 10 }, (_, index) => Object.assign(new Button(), { interactable: index < 4 }))
  const status = new (await import(moduleUrl('export class Label { constructor() { this.string = "" } }'))).Label()
  const selected = []
  let nextResult = { ok: true, stageNumber: 4 }

  bootstrap.worldStageSelectRoot = selector
  bootstrap.battleRoot = battle
  bootstrap.worldStageCloseNode = close
  bootstrap.worldStageEntryNode = entry
  bootstrap.worldStageItemNodes = items
  bootstrap.worldStageButtons = buttons
  bootstrap.worldStageStatusLabel = status
  bootstrap.worldStageController = {
    bind() {},
    select(stageId) {
      if (!buttons[stageId - 1]?.interactable) return false
      selector.emit('world-stage-selected', { stageId })
      return true
    },
  }
  bootstrap.battleRuntimeController = {
    advanceToStage(stageId) {
      selected.push(stageId)
      return nextResult
    },
  }
  bootstrap.refreshWorldStageSelection = () => {}

  bootstrap.bindWorldStageSelectorEvents()
  bootstrap.bindWorldStageSelectorEvents()
  assert.equal(entry.listenerCount('click'), 1)
  assert.equal(close.listenerCount('click'), 1)
  assert.equal(selector.listenerCount('world-stage-selected'), 1)
  assert.equal(items.every((item) => item.listenerCount('click') === 1), true)
  ...
  entry.emit('click')
  nextResult = { ok: false, stageNumber: 4 }
  items[2].emit('click')
  ...
  items[4].emit('click')
  assert.deepEqual(selected, [4, 3])
  selector.emit('world-stage-selection-rejected', { stageId: 5, reason: 'locked-stage' })
  assert.equal(selector.active, true)
  ...
  bootstrap.onDestroy()
  assert.equal(entry.listenerCount('click'), 0)
  assert.equal(close.listenerCount('click'), 0)
  assert.equal(selector.listenerCount('world-stage-selected'), 0)
  assert.equal(items.every((item) => item.listenerCount('click') === 0), true)
})
