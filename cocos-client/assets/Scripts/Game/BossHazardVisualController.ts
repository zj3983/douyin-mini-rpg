import { _decorator, Component, Graphics, Sprite, SpriteFrame, UITransform } from 'cc'

const { ccclass, property } = _decorator
const DEFAULT_LAYER_SIZE = 1

function positiveDimension(value: number): number {
  return Number.isFinite(value) && value > 0 ? value : DEFAULT_LAYER_SIZE
}
...
function setLayerSize(sprite: Sprite | null, width: number, height: number, factor: number): void {
  sprite?.node.getComponent(UITransform)?.setContentSize(width * factor, height * factor)
}
